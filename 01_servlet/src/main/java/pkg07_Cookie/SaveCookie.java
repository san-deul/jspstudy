...
public class SaveCookie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // 쿠키 이름과 값
	  // 서버가 만들어서 쿠키값은 클라이언트가 가지고 있도록 함
	  Cookie cookie1 = new Cookie("name", URLEncoder.encode(name,"UTF-8")); // 쿠키 값은 String 타입이다. 공백 등 쿠키값으로 사용할 수 없는 문자가 있어서 인코딩 후 저장한다.// 이름과 값
	  Cookie cookie2 =  new Cookie("email", URLEncoder.encode(email,"UTF-8"));  

	  // 쿠키 유지 시간
	  cookie1.setMaxAge(60 * 60 * 24 * 15 ); // 15일
	  // cookie2.setMaxAge(0);               // 쿠키 유지시간 생략하면 세션쿠기가 된다.(브라우저를 닫으면 지워지는 쿠키)
